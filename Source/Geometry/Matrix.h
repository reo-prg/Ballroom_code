...
Vector2<float> MultipleVec(const Matrix3& mat, const Vector2<float>& vec) {
	Vector2<float> ret = {};
	ret.x = mat.matrix_[0] * vec.x + mat.matrix_[1] * vec.y + mat.matrix_[2];
	ret.y = mat.matrix_[3] * vec.x + mat.matrix_[4] * vec.y + mat.matrix_[5];
	return ret;
}

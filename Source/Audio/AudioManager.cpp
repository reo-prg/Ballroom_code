...
{
	if (filenameTable_.contains(key))
	{
		return;
	}
...
int AudioManager::CreateSubmix(std::initializer_list<int> outputHandles)
{
	...
	for (auto& oh : outputHandles)
	{
		...
		stage = min(submix_[h]->stage_, stage);
	}
	...
	{
		int h = (oh & SubmixHandleMask) >> SubmixHandleShift;
		AddSubmixOutputTarget(index, oh);
	}

	return index;
}

int AudioManager::Play(const std::string& key, float volume, bool deleteIfEnd, std::initializer_list<int> outputHandles)
{
	if (filenameTable_.find(key) == filenameTable_.end())
	{
		OutputDebugStringA("key not found");
		return -1;
	}
	...
	auto data = wavLoader_->GetWAVFile(filenameTable_.at(key));
	...
	srcdata->deleteWhenEnd_ = deleteIfEnd;

	for (auto& sub : outputHandles)
	{
		AddSourceOutputTarget(index + SourceIdentifyID, sub);
	}
...
int AudioManager::PlayTemporary(const std::string& key, float volume, std::initializer_list<int> outputHandles)
{
	if (filenameTable_.find(key) == filenameTable_.end())
	{
		OutputDebugStringA("key not found");
...
int AudioManager::PlayLoop(const std::string& key, float begin, 
	float length, unsigned int loopCount, float volume, bool deleteIfEnd, std::initializer_list<int> outputHandles)
{
	if (filenameTable_.find(key) == filenameTable_.end())
	{
		OutputDebugStringA("key not found");
	...
	sdata->buffer_.PlayBegin = static_cast<UINT32>(static_cast<float>(sdata->waveFormat_.nSamplesPerSec) * begin);
	sdata->buffer_.PlayLength = static_cast<UINT32>(static_cast<float>(sdata->waveFormat_.nSamplesPerSec) * length);
	sdata->buffer_.LoopBegin = static_cast<UINT32>(static_cast<float>(sdata->waveFormat_.nSamplesPerSec) * begin);
	sdata->buffer_.LoopLength = static_cast<UINT32>(static_cast<float>(sdata->waveFormat_.nSamplesPerSec) * length);
	...
	sdata->handle_ = index;
	sdata->deleteWhenEnd_ = deleteIfEnd;
...
void AudioManager::PlayAgain(int handle, float begin, float length)
{
	...
	src->buffer_.PlayBegin = static_cast<UINT32>(static_cast<float>(src->waveFormat_.nSamplesPerSec) * begin);
	src->buffer_.PlayLength = static_cast<UINT32>(static_cast<float>(src->waveFormat_.nSamplesPerSec) * length);
...
float AudioManager::GetVolume(int handle)
{
	...
	{
		handle = handle & SourceHandleMask;
		if (handle >= SourceVoiceArrayMaxSize) { return 0.0f; }
		if (source_[handle])
		{
			source_[handle]->sourceVoice_->GetVolume(&vol);
		}
	}
	else if (id == SubmixIdentifyID)
	{
		handle = (handle & SubmixHandleMask) >> SubmixHandleShift;
		if (handle >= SubmixVoiceArrayMaxSize) { return 0.0f; }
		if (submix_[handle])
		{
			submix_[handle]->submixVoice_->GetVolume(&vol);
		}
	}
	return vol;
}
...
void AudioManager::Stop(int handle)
{
	...
	int srchandle = handle & SourceHandleMask;

	// 既に停止していたらなにもしない
	if (source_[srchandle]->vState_ == VoiceState::Stop) { return; }

	// 終了時に破棄するか
	if (source_[srchandle]->deleteWhenEnd_)
	{
		DeleteHandle(handle);
		return;
	}

	source_[srchandle]->sourceVoice_->Stop();
	source_[srchandle]->vState_ = VoiceState::Stop;
}
...
void AudioManager::StopAll(bool destroy)
{
	for (auto& h : source_.GetHandleList())
	{
		...
		{
			if (source_[h]->deleteWhenEnd_ || destroy)
			{
				DeleteHandle(h + SourceIdentifyID);
				continue;
			}
...
void AudioManager::Update(void)
{
	for (auto& s : source_.GetHandleList())
	{
		...
		{
			if (source_[s]->deleteWhenEnd_)
			{
				DeleteHandle(s + SourceIdentifyID);
				continue;
			}
...
void AudioManager::AddSourceOutputTarget(int sourceHandle, int targetHandle)
{
	...
	XAUDIO2_VOICE_SENDS snd = { static_cast<UINT32>(src->send_.size()), src->send_.data()};
...
void AudioManager::AddSubmixOutputTarget(int submixHandle, int targetHandle)
{
	...
	{ static_cast<UINT32>(sub->send_.size()), 
		sub->send_.data() };
...
void AudioManager::RemoveSourceOutputTarget(int sourceHandle, int targetHandle)
{
	...
	auto it1 = std::remove_if(src->output_.begin(), src->output_.end(),
		[&tgt](SubmixVoice* sub) { return sub->handle_ == tgt.get()->handle_; });
	...
	auto it3 = std::remove_if(tgt->sources_.begin(), tgt->sources_.end(),
		[&src](SourceVoice* sr) { return sr->handle_ == src.get()->handle_; });
	tgt->sources_.erase(it3, tgt->sources_.end());

	if (src->send_.size() == 0 && targetHandle != RootSubmixHandle)
	{
...
void AudioManager::RemoveSubmixOutputTarget(int submixHandle, int targetHandle)
{
	...
	auto it3 = std::remove_if(tgt->input_.begin(), tgt->input_.end(),
		[&sub](SubmixVoice* sr) { return sr->handle_ == sub.get()->handle_; });
	...
	XAUDIO2_VOICE_SENDS snd = { static_cast<UINT32>(sub->send_.size()), sub->send_.data() };
	sub->submixVoice_->SetOutputVoices(&snd);
}
...
int AudioManager::AddEffect(int handle, AudioEffectType type, bool active, int insertPosition)
{
	...
	XAUDIO2_EFFECT_CHAIN chain = { static_cast<UINT32>(submix_[hd]->efkDesc_.size()),
...
void AudioManager::SetMasteringLimiterParameter(UINT32 release, UINT32 loudness, int submixHandle, int effectIndex)
{
	if (!SubmixHandleIsValid(submixHandle)) { return; }
	submixHandle = (submixHandle & SubmixHandleMask) >> SubmixHandleShift;
	if (effectIndex >= submix_[submixHandle]->efkDesc_.size()) { return; }

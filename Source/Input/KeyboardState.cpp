...
KeyboardState::KeyboardState()
{
	...
	for (int j = 0; j < 256; j++)
	{
		state_[1][j] = static_cast<unsigned char>(0x80);
	}
}
...
void KeyboardState::Update(void)
{
	...
	{
		OutputDebugString("Error on Get KeyState");
	}
}
...
bool KeyboardState::Triggered(unsigned int button)
{
	...
	return (Pressed(button) && (!Pressed(button, 1 - static_cast<unsigned int>(currentBuffer_))));
}
